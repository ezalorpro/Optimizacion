...
class Rastrigin(metaclass=OrderedClassMembers):
	...
	@staticmethod
	def calcular_fitness_rastrigin(x: 'List[Tuple[float, float,...,dimension]]', _) -> float:
		"""
		...
		x = np.array(*x)
		fitness = sum([xi ** 2 - 10 * np.cos(2 * np.pi * xi) + 10 for xi in x])
		return fitness
	...
	def imprimir_respuesta(self, _, mejor_posicion: object, cola: None = None) -> None:
		...
		:param _: None
		:param mejor_posicion: Mejor valor encontrado para la funcion Rastrigin.
		...
		if self.dimension == 2:
			...
			z = sum([xi ** 2 - 10 * np.cos(2 * np.pi * xi) + 10 for xi in [x, y]])
			xp = np.array(*mejor_posicion.position)
			zp = sum([xi ** 2 - 10 * np.cos(2 * np.pi * xi) + 10 for xi in xp])
